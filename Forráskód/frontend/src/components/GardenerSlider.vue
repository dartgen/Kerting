...
<template>
  <div class="w-full flex flex-col justify-center relative py-2 lg:py-0 h-full">
    ...
    <div v-else class="flex justify-center items-center gap-2 md:gap-6 h-full relative w-full perspective-1000 min-h-[280px]">
      ...
      >
        <div
          class="w-16 h-16 md:w-20 md:h-20 lg:w-24 lg:h-24 flex items-center justify-center rounded-full mb-2 md:mb-4 text-2xl md:text-4xl overflow-hidden shadow-lg shrink-0 border-4"
          :class="getCardStyle(index).scale === 1.1 ? 'bg-earth-100 text-earth-600 border-earth-200' : 'bg-earth-800 text-earth-200 border-earth-700'"
        >
          <img
        ...
        <h3 class="text-sm md:text-xl font-bold mb-1 text-earth-50 truncate w-full text-center">{{ pro.name }}</h3>
        ...
        <div class="mb-1 md:mb-2 text-xs md:text-lg w-full flex flex-col items-center">
          <div v-if="pro.ertekelesSzam > 0" class="flex items-center gap-1 text-yellow-500">
            <span class="text-earth-100 text-xs md:text-sm font-semibold mr-1">{{ pro.ertekeles.toFixed(1) }}</span>
          ...
          <div v-else class="text-earth-300/70 text-[10px] md:text-xs italic">
            Még nincs értékelés
          </div>
        </div>

        <p class="text-[10px] md:text-sm text-earth-100/85 text-left line-clamp-3 md:line-clamp-4 leading-snug md:leading-relaxed mb-auto w-full px-1 md:px-2">{{ pro.desc }}</p>

        <button
          class="mt-2 md:mt-4 w-full py-1.5 md:py-2 uppercase text-[10px] md:text-sm font-semibold tracking-[0.12em] bg-earth-800/90 border border-earth-300/30 text-earth-50 rounded-lg hover:bg-earth-700 hover:border-earth-200/50 transition-all shadow-lg"
